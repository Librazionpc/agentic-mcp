...
async def _proxy_tool(caller: CallerContext, args: dict, tool_name: str) -> dict:
    routes = _load_routes()
    prefix = str(routes.get("backend_prefix", "NOTIFICATION")).strip() or "NOTIFICATION"
...
def main() -> None:
    send = {"comms.send"}
    tools = [
        _tool("send_notification", send),
        _tool("payment_reminder_send", send),
        _tool("reminder_schedule_create", send),
        _tool("notify_team_slack", send),
        _tool("notification_send", send),
        _tool("send_message", send),
    ]

    app = create_app("notification-mcp", tools)
    host = os.environ.get("NOTIFICATION_MCP_HOST", "0.0.0.0")
    port = int(os.environ.get("NOTIFICATION_MCP_PORT", "8096"))
