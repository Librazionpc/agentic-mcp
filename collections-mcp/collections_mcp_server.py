...
async def _proxy_tool(caller: CallerContext, args: dict, tool_name: str) -> dict:
    routes = _load_routes()
    prefix = str(routes.get("backend_prefix", "LOANS")).strip() or "LOANS"
...
def main() -> None:
    ...
    tools = [
        _tool("debt_status_check", read),
        _tool("repayment_schedule_update", write),
        _tool("account_restriction_apply", write),
        _tool("restriction_lookup", read),
        _tool("risk_score_update", write),
    ]

    app = create_app("collections-mcp", tools)
    host = os.environ.get("COLLECTIONS_MCP_HOST", "0.0.0.0")
    port = int(os.environ.get("COLLECTIONS_MCP_PORT", "8100"))
