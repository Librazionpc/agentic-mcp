...
async def _proxy_tool(caller: CallerContext, args: dict, tool_name: str) -> dict:
    routes = _load_routes()
    prefix = str(routes.get("backend_prefix", "ESCALATION")).strip() or "ESCALATION"
...
def main() -> None:
    tools = [
        _tool("create_human_ticket", {"ticket.escalate"}),
        _tool("escalation_log_create", {"ticket.escalate"}),
        _tool("incident_report_generate", {"incident.read"}),
        _tool("ticket_escalate", {"ticket.escalate"}),
        _tool("incident_status_lookup", {"incident.read"}),
    ]

    app = create_app("escalation-mcp", tools)
    host = os.environ.get("ESCALATION_MCP_HOST", "0.0.0.0")
    port = int(os.environ.get("ESCALATION_MCP_PORT", "8097"))
